# Next.js Project Setup and Basics

This repository **alx-project-0x02-setup** provides the initial scaffolding for a modern web application using **Next.js**, **TypeScript**, and **Tailwind CSS**. It is designed as a foundational setup to help developers quickly spin up projects with clean structure, type safety, and responsive styling.

## 🎯 Objective

The goal of this repository is to establish a baseline project environment that demonstrates:

- Scaffold a Next.js project with TypeScript and Tailwind CSS.
- Enable ESLint for code quality enforcement.
- Create a modular folder structure for components, interfaces, and assets.
- Add a simple welcome message to verify the setup.

## 📦 Features

- **Next.js** for server-side rendering and routing.
- **TypeScript** for type safety and developer productivity.
- **Tailwind CSS** for utility-first styling and responsive design.
- **ESLint + Prettier** for consistent code quality and formatting.

## 🗂️ Project Structure (Initial)

```folder
alx-project-0x02/
├── components/
│   └── layout/
│       └── Header.tsx
├── interfaces/
│   └── index.ts
├── pages/
│   ├── index.tsx
│   ├── _app.tsx
│   └── _document.tsx
├── public/
│   └── assets/
│       └── images/
├── styles/
│   └── globals.css
├── README.md
├── package.json
├── tsconfig.json
└── tailwind.config.js

```

## 🚀 Getting Started

### 1. Clone the repository

```bash
git clone https://github.com/D0nG4667/alx-project-0x02-setup.git
cd alx-project-0x02-setup
cd alx-project-0x02
```

### 2. Install dependencies

```bash
npm install
# or
yarn install
```

### 3. Run the development server

```bash
npm run dev
# or
yarn dev
```

Open [http://localhost:3000](http://localhost:3000) in your browser to see the app.

---

## 👤 Author

- GitHub: [@D0nG4667](https://github.com/D0nG4667)  
- LinkedIn: [Gabriel Okundaye](https://www.linkedin.com/in/dr-gabriel-okundaye)

---

## 🌟 Show Your Support

If this project helps you get started with Next.js, give it a ⭐️ to support future open-source work.
